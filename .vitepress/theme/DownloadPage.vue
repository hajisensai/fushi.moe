...
import { probeSources, downloadChunked, createBrowserSink, verifySink } from './chunked-download.mjs'
import { PACK_FILENAME, downloadPack, fetchPackManifest, preferredPartBaseUrl } from './pack-download.mjs'
...
/**
 * 每种语言各有一份 <lang>/download.md（tool/build_download_pages.mjs 生成），把该语言的字典
 * 整份传进来：SSR 时 t() 就是该语言，静态 HTML 里没有一句中文占位；访客在页面上切换语言
 * 后由 site.js 广播新字典，t() 跟着换。ZH 是字典缺键时的兜底（标记源语言原文）。
 */
const props = defineProps({
  lang: { type: String, required: true },
  dict: { type: Object, required: true },
})
const { t } = useSiteI18n({ lang: props.lang, dict: props.dict })
const ZH = {
  "dl.title": "下载 Fushi",
  "dl.lead": "选一个平台装上，剩下的交给应用内的新手引导。四个平台同一套体验，全部免费开源。",
  "dl.linux": "Linux 暂无预编译包，请<a href=\"https://github.com/hajisensai/Fushi#building\">从源码构建</a>。",
  "dl.history": "历史版本见<a href=\"https://github.com/hajisensai/Fushi/releases\">全部 Releases</a>。",
  "dl.after_eyebrow": "装好之后",
  "dl.guide_title": "跟着新手引导走就行",
  "dl.guide_p1": "第一次启动 Fushi 会自动打开新手引导：先问你想用哪些功能，再一步步把推荐词典包、Anki、备份同步和阅读字体配好。中途关掉了想重来，路径是 <strong>设置 → 系统 → 新手引导</strong>。",
  "dl.guide_th_step": "引导步骤",
  "dl.guide_th_what": "做什么",
  "dl.guide_r1_a": "欢迎",
  "dl.guide_r1_b": "选界面语言和明暗主题",
  "dl.guide_r2_a": "功能选择",
  "dl.guide_r2_b": "勾掉用不上的模块，它们不会出现在底栏",
  "dl.guide_r3_a": "推荐包",
  "dl.guide_r3_b": "下载并导入日语词典 + 日/英发音音频库",
  "dl.guide_r4_a": "Anki",
  "dl.guide_r4_b": "连上 AnkiDroid / AnkiMobile / AnkiConnect，一键创建 Lapis 牌组",
  "dl.guide_r5_a": "备份 · 互联 · 字体",
  "dl.guide_r5_b": "按需配置，都可以跳过",
  "dl.pack_title": "推荐包：不用一本一本挑词典",
  "dl.pack_p1": "推荐包里是<strong>日语单词、音调、词频词典</strong>，外加<strong>日语 / 英语本地发音音频库</strong>，约 9.5 GB。在引导的「安装推荐包」那一步里直接下载并导入。",
  "dl.pack_warn_title": "导入方式选「合并」",
  "dl.pack_warn_body": "推荐包走的是备份导入流程，确认框里选<strong>「合并到现有库」</strong>只会加进词典和发音库，不动你已有的数据；<strong>「覆盖整库」</strong>会用推荐包替换整个库。",
  "dl.pack_p2": "学别的语言？跳过这一步，用词典管理导入任意 Yomitan / MDX (MDict) / DSL 格式的词典即可。可查语言由你导入的词典决定。"
}
/** @param {string} key @returns {string} */
const tz = (key) => t(key, ZH[key])
...
<p v-html="tz('dl.linux')"></p>

<p v-html="tz('dl.history')"></p>
...
<p class="eyebrow">{{ tz('dl.after_eyebrow') }}</p>

<h2>{{ tz('dl.guide_title') }}</h2>

<p v-html="tz('dl.guide_p1')"></p>

<table>
<thead><tr><th>{{ tz('dl.guide_th_step') }}</th><th>{{ tz('dl.guide_th_what') }}</th></tr></thead>
<tbody>
<tr><td>{{ tz('dl.guide_r1_a') }}</td><td>{{ tz('dl.guide_r1_b') }}</td></tr>
<tr><td>{{ tz('dl.guide_r2_a') }}</td><td>{{ tz('dl.guide_r2_b') }}</td></tr>
<tr><td>{{ tz('dl.guide_r3_a') }}</td><td>{{ tz('dl.guide_r3_b') }}</td></tr>
<tr><td>{{ tz('dl.guide_r4_a') }}</td><td>{{ tz('dl.guide_r4_b') }}</td></tr>
<tr><td>{{ tz('dl.guide_r5_a') }}</td><td>{{ tz('dl.guide_r5_b') }}</td></tr>
</tbody>
</table>

<h2>{{ tz('dl.pack_title') }}</h2>

<p v-html="tz('dl.pack_p1')"></p>
...
<p class="custom-block-title">{{ tz('dl.pack_warn_title') }}</p>
<p v-html="tz('dl.pack_warn_body')"></p>
</div>
