 ...
 * 会被 pushState 成它自己的 404 页。vp-raw 内的链接它放行（底栏「怎么开始」指向的 /immersion
 * 虽是 VitePress 页，在 vp-raw 里同样走整页导航，行为和其它链接一致）。
 */
import { Content, useData } from 'vitepress'
import { onMounted } from 'vue'
import { routeFor } from './lang-routes.mjs'
...
<template>
  <div class="site-shell">
    <nav class="site-nav vp-raw" id="top">
      <a class="site-nav-brand" :href="routeFor(lang, '/')"><span class="logo" aria-hidden="true">F</span>Fushi</a>
    ...
    <a class="site-totop" href="#top" :aria-label="c('nav.totop', '回到顶部')" :title="c('nav.totop', '回到顶部')" data-i18n-attr="aria-label=nav.totop;title=nav.totop"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M12 19V5M5 12l7-7 7 7"/></svg></a>
  </div>
</template>
