<script setup>
/*
 * 站点页壳——顶栏 / 底栏与静态首页 public/index.html 完全同形，样式同出
 * /chrome.css。以前这里跑的是 VitePress 默认文档主题（自带侧边栏、搜索框、
 * 上一页/下一页），跟首页是两套观感，从首页点进 /download 直接断裂。
 *
 * 标记是从 public/index.html 那份抄过来的：两处改动必须同步，判据见
 * public/chrome.css 末尾的「标记形状」注释。
 *
 * 站内跳转：VitePress 页之间（下载 / 沉浸 / 常见问题）走客户端路由，不整页刷新；
 * 只有首页是静态 public/index.html、不在路由表里——点 logo（/）会被 pushState 成它自己的 404 页，
 * 所以 logo 链接单独带 vp-raw（VitePress 放行 .vp-raw 里的链接，走整页导航）。
 * 页壳在路由切换后不重新挂载，所以路由切换时要自己补三件事（见下面的 watch）：
 * 收起窄屏菜单、展开锚点指到的问答、重新应用界面语言。
 */
import { Content, useData, useRoute } from 'vitepress'
import { computed, nextTick, onMounted, onUnmounted, watch } from 'vue'
import { RTL, routeFor } from './lang-routes.mjs'
import { useSiteI18n } from './i18n.js'
import FaqSidebar from './FaqSidebar.vue'
import FaqOutline from './FaqOutline.vue'

/*
 * 站内链接指到本页 locale 语言的版本（目录即 locale：/de/… 下的页面 lang 是 de）。
 * 访客在页面上切换语言时由 site.js 按同一张表改 href。
 */
const { lang, theme, frontmatter, page } = useData()
/**
 * 页壳文案按本页 locale 烤进 SSR（config.mts 把各语言字典里的这几句放进 themeConfig.chrome）；
 * 元素上仍带同名 data-i18n / data-i18n-attr 键，访客切语言时 site.js 照旧按键替换。
 * @param {string} key
 * @param {string} zh 字典缺键时的兜底（源语言原文）
 * @returns {string}
 */
const c = (key, zh) => (theme.value.chrome && typeof theme.value.chrome[key] === 'string' ? theme.value.chrome[key] : zh)

/*
 * 界面语言由 /site.js（config.mts 的 head 里以 data-manual 挂载）负责；这里只在
 * hydrate 完成后触发一次应用。hydrate 之前动 DOM 文本，Vue 会按服务端 vnode 把它
 * 改回去；hydrate 之后静态内容不再被 patch，data-i18n 的替换才站得住。
 * 选源器那类响应式区域不走 data-i18n，走 useSiteI18n() 的 t()。
 */
/**
 * 构建产物里 /site.js 同步挂在 <head>，mounted 时一定在；dev 模式下 VitePress 把 head 里的 script
 * 在客户端动态插入，可能比页壳 mounted 晚几十毫秒——那就等它出现再应用，否则 dev 里首屏语言不对。
 * @param {number} tries 剩余重试次数（每 100ms 一次）
 */
const applyI18n = (tries = 30) => {
  const i18n = typeof window !== 'undefined' ? window.fushiI18n : undefined
  if (i18n) i18n.apply()
  else if (tries > 0) setTimeout(() => applyI18n(tries - 1), 100)
  // 主题钮的 aria-pressed 是 site.js 在 <head> 里就写好的，而页壳 SSR 出来的是静态的
  // aria-pressed="false"——hydrate 会按 vnode 把它抹回去。这里跟着补一次。
  const t = typeof window !== 'undefined' ? window.fushiTheme : undefined
  if (t) t.apply()
}
// 等一个 tick：VitePress 自己的根组件在 mounted 里才把 <html lang> 设成本页 locale（父组件的 mounted 排在
// 页壳之后），site.js 靠这个属性判断页面自带的语言。dev 模式的 HTML 模板没有 lang，不等就会误判成中文源页。
onMounted(() => { nextTick(() => applyI18n()) })

/*
 * 站内路由切换：新页面的标记刚 patch 进 DOM（flush: 'post'），页壳还是同一个实例。
 * VitePress 已把 <html lang> 改成新页 locale 的语言，site.js 的 apply() 据此判断要不要换文案
 * （中文访客进英文 locale 的 /faq 就换，进 /zh-cn/immersion 就不用）。
 */
const route = useRoute()
watch(() => route.path, async () => {
  await nextTick()
  const check = document.getElementById('site-nav-check')
  if (check) check.checked = false
  openHashDetails()
  applyI18n()
}, { flush: 'post' })

/*
 * 常见问题的文章（faq/<slug>.md）：标题（= 问题）/ 更新日期 / 返回列表由页壳渲染，正文 .md 里只写答案。
 * 常见问题的两种页（列表 faq.md、文章）套文档式布局：左侧分组目录 FaqSidebar.vue，文章页右侧再挂本页标题 FaqOutline.vue。
 * 常见问题不走语言路由，文章各写各的语言：正文容器挂文章自己的 lang / dir，页壳文案照旧走 data-i18n。
 * 日期按当前界面语言格式化（useSiteI18n 跟着 site.js 的 fushi:i18n 广播换语言）。
 */
const isFaqArticle = computed(() => /^faq\/[^/]+\.md$/.test(page.value.relativePath))
/** 列表页 + 文章页都套文档式三栏：左侧目录（FaqSidebar）/ 正文 / 右侧本页内容（FaqOutline，只有文章页有） */
const isFaq = computed(() => page.value.relativePath === 'faq.md' || isFaqArticle.value)
const articleLang = computed(() => (isFaqArticle.value ? String(frontmatter.value.lang || 'zh-CN') : undefined))
const articleDate = computed(() => (isFaqArticle.value && frontmatter.value.date ? String(frontmatter.value.date).slice(0, 10) : ''))
const { lang: uiLang } = useSiteI18n({ lang: lang.value })
const articleDateText = computed(() => {
  if (!articleDate.value) return ''
  try {
    return new Intl.DateTimeFormat(uiLang.value, { dateStyle: 'long', timeZone: 'UTC' }).format(new Date(articleDate.value + 'T00:00:00Z'))
  } catch (_) {
    return articleDate.value
  }
})

/* 带锚点打开一个 <details>（/download#xxx 这类）时把它展开，否则跳过去只看到一行收着的标题。 */
function openHashDetails() {
  const id = location.hash.length > 1 ? decodeURIComponent(location.hash.slice(1)) : ''
  const el = id ? document.getElementById(id) : null
  if (el && el.tagName === 'DETAILS') el.open = true
}
onMounted(() => {
  openHashDetails()
  window.addEventListener('hashchange', openHashDetails)
})
onUnmounted(() => {
  if (typeof window !== 'undefined') window.removeEventListener('hashchange', openHashDetails)
})
</script>

<template>
  <div class="site-shell">
    <nav class="site-nav" id="top">
      <a class="site-nav-brand vp-raw" :href="routeFor(lang, '/')"><span class="logo" aria-hidden="true">F</span>Fushi</a>
      <div class="site-nav-right">
        <input class="site-nav-check" type="checkbox" id="site-nav-check" :aria-label="c('nav.menu', '展开菜单')" data-i18n-attr="aria-label=nav.menu">
        <label class="site-nav-burger" for="site-nav-check" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true"><path d="M4 7h16M4 12h16M4 17h16"/></svg></label>
        <span class="site-nav-links">
          <a :href="routeFor(lang, '/immersion')"><span class="site-nav-word" data-i18n="nav.method">{{ c('nav.method', '怎么开始') }}</span></a>
          <a href="/faq"><span class="site-nav-word" data-i18n="nav.faq">{{ c('nav.faq', '常见问题') }}</span></a>
          <a class="site-nav-ico" href="https://qm.qq.com/q/Sx2nWTvJCw" :title="c('nav.qq', 'QQ 群')" data-i18n-attr="title=nav.qq"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M21.395 15.035a40 40 0 0 0-.803-2.264l-1.079-2.695c.001-.032.014-.562.014-.836C19.526 4.632 17.351 0 12 0S4.474 4.632 4.474 9.241c0 .274.013.804.014.836l-1.08 2.695a39 39 0 0 0-.802 2.264c-1.021 3.283-.69 4.643-.438 4.673.54.065 2.103-2.472 2.103-2.472 0 1.469.756 3.387 2.394 4.771-.612.188-1.363.479-1.845.835-.434.32-.379.646-.301.778.343.578 5.883.369 7.482.189 1.6.18 7.14.389 7.483-.189.078-.132.132-.458-.301-.778-.483-.356-1.233-.646-1.846-.836 1.637-1.384 2.393-3.302 2.393-4.771 0 0 1.563 2.537 2.103 2.472.251-.03.581-1.39-.438-4.673"/></svg><span class="site-nav-word" data-i18n="nav.qq">{{ c('nav.qq', 'QQ 群') }}</span></a>
          <a class="site-nav-ico" href="https://discord.gg/WhjwyGmm7f" title="Discord"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.4189-2.1568 2.4189Z"/></svg><span class="site-nav-word">Discord</span></a>
          <a class="site-nav-ico" href="https://github.com/hajisensai/Fushi" title="GitHub"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg><span class="site-nav-word">GitHub</span><span class="site-nav-star" data-fushi-stars-host aria-hidden="true"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M12 2.6l2.95 5.98 6.6.96-4.78 4.66 1.13 6.57L12 17.67l-5.9 3.1 1.13-6.57L2.45 9.54l6.6-.96z"/></svg><span data-fushi-stars="compact"></span></span></a>
        </span>
        <div class="site-nav-lang" :title="c('nav.language', '语言')" data-i18n-attr="title=nav.language">
          <button class="site-nav-lang-btn" type="button" aria-haspopup="listbox" aria-expanded="false" :aria-label="c('nav.language', '语言')" data-i18n-attr="aria-label=nav.language"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20zm7.93 9h-3.02a15.6 15.6 0 0 0-1.3-5.3A8.03 8.03 0 0 1 19.93 11zM12 4.04c.83 1.2 1.87 3.45 2.13 6.96H9.87c.26-3.51 1.3-5.76 2.13-6.96zM4.07 13h3.02c.12 1.94.6 3.75 1.3 5.3A8.03 8.03 0 0 1 4.07 13zm3.02-2H4.07a8.03 8.03 0 0 1 4.32-5.3c-.7 1.55-1.18 3.36-1.3 5.3zM12 19.96c-.83-1.2-1.87-3.45-2.13-6.96h4.26c-.26 3.51-1.3 5.76-2.13 6.96zm3.61-1.66c.7-1.55 1.18-3.36 1.3-5.3h3.02a8.03 8.03 0 0 1-4.32 5.3z"/></svg><span class="site-nav-lang-current">{{ theme.langName || '简体中文' }}</span><svg class="site-nav-lang-chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 9l6 6 6-6"/></svg></button>
          <ul class="site-nav-lang-menu" role="listbox" hidden>
            <li><button type="button" role="option" data-lang="auto"><span data-i18n="nav.lang_auto">{{ c('nav.lang_auto', '自动') }}</span><span class="site-nav-lang-sub">{{ theme.langName || '简体中文' }}</span></button></li>
            <li><button type="button" role="option" data-lang="zh-CN">简体中文</button></li>
            <li><button type="button" role="option" data-lang="zh-HK">繁體中文</button></li>
            <li><button type="button" role="option" data-lang="en">English</button></li>
            <li><button type="button" role="option" data-lang="ja">日本語</button></li>
            <li><button type="button" role="option" data-lang="ko">한국어</button></li>
            <li><button type="button" role="option" data-lang="de">Deutsch</button></li>
            <li><button type="button" role="option" data-lang="es">Español</button></li>
            <li><button type="button" role="option" data-lang="fr">Français</button></li>
            <li><button type="button" role="option" data-lang="it">Italiano</button></li>
            <li><button type="button" role="option" data-lang="nl">Nederlands</button></li>
            <li><button type="button" role="option" data-lang="pt-BR">Português (Brasil)</button></li>
            <li><button type="button" role="option" data-lang="ru">Русский</button></li>
            <li><button type="button" role="option" data-lang="tr">Türkçe</button></li>
            <li><button type="button" role="option" data-lang="vi">Tiếng Việt</button></li>
            <li><button type="button" role="option" data-lang="th">ไทย</button></li>
            <li><button type="button" role="option" data-lang="id">Bahasa Indonesia</button></li>
            <li><button type="button" role="option" data-lang="ar">العربية</button></li>
          </ul>
        </div>
        <button class="site-nav-theme" type="button" aria-pressed="false" :title="c('nav.theme', '深色模式')" :aria-label="c('nav.theme', '深色模式')" data-i18n-attr="title=nav.theme;aria-label=nav.theme"><svg class="site-nav-theme-sun" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M12 17a5 5 0 1 1 0-10 5 5 0 0 1 0 10zm0-13.2a1 1 0 0 1-1-1V1.6a1 1 0 0 1 2 0v1.2a1 1 0 0 1-1 1zm0 19.6a1 1 0 0 1-1-1v-1.2a1 1 0 0 1 2 0v1.2a1 1 0 0 1-1 1zM22.4 13h-1.2a1 1 0 0 1 0-2h1.2a1 1 0 0 1 0 2zm-19.6 0H1.6a1 1 0 0 1 0-2h1.2a1 1 0 0 1 0 2zm16.53-7.33-.85.85a1 1 0 0 1-1.41-1.41l.85-.85a1 1 0 0 1 1.41 1.41zM6.08 19.34l-.85.85a1 1 0 1 1-1.41-1.41l.85-.85a1 1 0 0 1 1.41 1.41zm12.85.85-.85-.85a1 1 0 0 1 1.41-1.41l.85.85a1 1 0 0 1-1.41 1.41zM5.23 6.52l-.85-.85a1 1 0 0 1 1.41-1.41l.85.85a1 1 0 0 1-1.41 1.41z"/></svg><svg class="site-nav-theme-moon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M21.3 14.4a1 1 0 0 0-1.15-.3 7.4 7.4 0 0 1-2.76.53 7.32 7.32 0 0 1-7.32-7.32c0-.95.18-1.88.53-2.76a1 1 0 0 0-1.23-1.33A9.72 9.72 0 0 0 2.6 12.4a9.72 9.72 0 0 0 9.72 9.72 9.72 9.72 0 0 0 9.18-6.57 1 1 0 0 0-.2-1.15z"/></svg></button>
        <a class="btn" :href="routeFor(lang, '/download')" data-i18n="nav.download">{{ c('nav.download', '下载') }}</a>
      </div>
    </nav>

    <main class="site-main" :class="{ 'site-main-faq': isFaq }">
      <div :class="isFaq ? 'faq-layout' : undefined">
        <FaqSidebar v-if="isFaq" />
        <article class="prose" :class="{ 'faq-article': isFaqArticle }" :lang="articleLang" :dir="articleLang ? (RTL[articleLang] ? 'rtl' : 'ltr') : undefined">
          <header v-if="isFaqArticle" class="faq-article-head">
            <p class="faq-article-nav"><a href="/faq" data-i18n="faq.back">{{ c('faq.back', '← 全部问题') }}</a></p>
            <h1>{{ frontmatter.title }}</h1>
            <p v-if="articleDate" class="faq-article-meta"><span data-i18n="faq.updated">{{ c('faq.updated', '更新于') }}</span> <time :datetime="articleDate">{{ articleDateText }}</time></p>
          </header>
          <Content />
          <p v-if="isFaqArticle" class="faq-more" data-i18n="faq.more" v-html="c('faq.more', '没找到答案？来 <a href=&quot;https://discord.gg/WhjwyGmm7f&quot;>Discord</a> 或 <a href=&quot;https://qm.qq.com/q/Sx2nWTvJCw&quot;>QQ 群</a>问。')"></p>
        </article>
        <FaqOutline v-if="isFaqArticle" />
      </div>
    </main>

    <footer class="site-footer">
      <nav class="site-footer-links">
        <a :href="routeFor(lang, '/immersion')" data-i18n="nav.method">{{ c('nav.method', '怎么开始') }}</a><a href="/faq" data-i18n="nav.faq">{{ c('nav.faq', '常见问题') }}</a><a :href="routeFor(lang, '/download')" data-i18n="nav.download">{{ c('nav.download', '下载') }}</a><a class="site-footer-ico" href="https://github.com/hajisensai/Fushi" title="GitHub" aria-label="GitHub"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg></a><a class="site-footer-ico" href="https://discord.gg/WhjwyGmm7f" title="Discord" aria-label="Discord"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.4189-2.1568 2.4189Z"/></svg></a><a class="site-footer-ico" href="https://qm.qq.com/q/Sx2nWTvJCw" :title="c('nav.qq', 'QQ 群')" :aria-label="c('nav.qq', 'QQ 群')" data-i18n-attr="title=nav.qq;aria-label=nav.qq"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M21.395 15.035a40 40 0 0 0-.803-2.264l-1.079-2.695c.001-.032.014-.562.014-.836C19.526 4.632 17.351 0 12 0S4.474 4.632 4.474 9.241c0 .274.013.804.014.836l-1.08 2.695a39 39 0 0 0-.802 2.264c-1.021 3.283-.69 4.643-.438 4.673.54.065 2.103-2.472 2.103-2.472 0 1.469.756 3.387 2.394 4.771-.612.188-1.363.479-1.845.835-.434.32-.379.646-.301.778.343.578 5.883.369 7.482.189 1.6.18 7.14.389 7.483-.189.078-.132.132-.458-.301-.778-.483-.356-1.233-.646-1.846-.836 1.637-1.384 2.393-3.302 2.393-4.771 0 0 1.563 2.537 2.103 2.472.251-.03.581-1.39-.438-4.673"/></svg></a>
      </nav>
      <p class="site-footer-sponsor"><a href="https://github.com/sponsors/hajisensai" target="_blank" rel="noopener noreferrer" data-i18n="footer.sponsor">{{ c('footer.sponsor', '通过 GitHub Sponsors 支持 Fushi') }}</a></p>
      <p class="site-footer-legal" data-i18n="footer.legal" v-html="c('footer.legal', '以 <a href=&quot;https://www.gnu.org/licenses/gpl-3.0.html&quot;>GPLv3</a> 许可发布')"></p>
      <p>© 2026 Fushi</p>
    </footer>
    <a class="site-totop vp-raw" href="#top" :aria-label="c('nav.totop', '回到顶部')" :title="c('nav.totop', '回到顶部')" data-i18n-attr="aria-label=nav.totop;title=nav.totop"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M12 19V5M5 12l7-7 7 7"/></svg></a>
  </div>
</template>
